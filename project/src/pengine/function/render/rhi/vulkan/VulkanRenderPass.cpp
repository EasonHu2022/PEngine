...
namespace pengine
{
	...
	inline auto getAttachmentDescription(std::shared_ptr<Texture> texture, bool clear = true) -> VkAttachmentDescription
	{
		...
		{
			attachment.format = VKHelper::getDepthFormat();
		...
		{
			PLOGE("[VULKAN] - Unsupported TextureType - {0}", int32_t(texture->getType()));
			return attachment;
		}
	...
	auto VulkanRenderPass::init(const RenderPassInfo& info) -> void
	{
		...
		for (uint32_t i = 0; i < info.attachments.size(); i++)
		{
			...
			{
				PLOGW("CubeMap could not be used as an attachment");
				continue;
			}
			...
			{
				PLOGE("Unsupported texture attachment");
			}
		}
		...
		VkRenderPassCreateInfo renderPassCreateInfo = VKHelper::renderPassCreateInfo();
	...
	auto VulkanRenderPass::beginRenderPass(CommandBuffer* commandBuffer,
		const glm::vec4& clearColor, FrameBuffer* frame, SubPassContents contents,
		uint32_t width, uint32_t height, int32_t cubeFace, int32_t mipMapLevel) const -> void
	{
		if (!depthOnly)
		{
