...
namespace pengine
{
	...
	namespace
	{
		...
		inline auto loadMesh(tinygltf::Model& model, tinygltf::Mesh& mesh, std::vector<std::shared_ptr<Material>>& materials, component::Transform& parentTransform) -> std::vector<std::shared_ptr<Mesh>>
		{
			...
			for (auto& primitive : mesh.primitives)
			{
...

				{
					...
					{
						PLOGW("Unsupported indices data type - {0}", componentTypeByteSize);
					}
				}
		...
		inline auto loadNode(const std::string& parentName, int32_t nodeIndex, const glm::mat4& parentTransform, tinygltf::Model& model, std::vector<std::shared_ptr<Material>>& materials, std::unordered_map<std::string, std::shared_ptr<Mesh>>& outMeshes)
		{
			...
			PLOGI("GLTF.asset.copyright : {0}", model.asset.copyright);
			PLOGI("GLTF.asset.generator : {0}", model.asset.generator);
			PLOGI("GLTF.asset.version : {0}", model.asset.version);
			PLOGI("GLTF.asset.minVersion : {0}", model.asset.minVersion);
